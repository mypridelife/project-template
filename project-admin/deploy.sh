...
APP_PORT=80          # 应用端口
HEALTH_CHECK_URL=http://crm-admin.hzhengji.cn  # 应用健康检查URL
HEALTH_CHECK_FILE_DIR=/home/admin/status   # 脚本会在这个目录下生成nginx-status文件
APP_HOME=/home/admin/crm-admin/dist  # 从package.tgz中解压出来的jar包放到这个目录下
...
start_application() {
    ...
    mv /home/admin/crm-admin/dist /usr/local/openresty/nginx/html/crm-admin-${dstr}
    rm -f /usr/local/openresty/nginx/html/crm-admin
    ln -s /usr/local/openresty/nginx/html/crm-admin-${dstr} /usr/local/openresty/nginx/html/crm-admin
}
stop_application() {
    echo ''
}
