...
const { exam, answer, upload, submit } = require('../../config')
...
Page({
  ...
  async handleNavigate() {
    var that = this
    const paperId = that.data.paperId
    that.setData({
      visible2: false
    })
    $Toast({
    ...
    wx.request({
      ...
      success(res) {
        ...
        $Toast({
          content: '您已通过正式题，3秒钟之后跳转到奖励页面',
          type: 'loading',
          duration: 3
        })
        setTimeout(() => {
          wx.navigateTo({
            url: `/pages/beforefinish/beforefinish?paperId=${paperId}`
          })
        }, 3000)
      }
    })
